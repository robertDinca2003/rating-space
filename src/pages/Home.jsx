...
const Home = () => {
    ...
    return (
        <div className="gap-10 h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-700 via-gray-900 to-black  flex flex-col justify-center items-center">
            <h1 className="outline-text text-8xl hover:text-white transition-colors duration-300 ">Rating Space</h1>
            <button onClick={HostRoom} className="w-[280px] text-white border-2 px-8 py-1 text-2xl transition-colors duration-300 hover:text-slate-600 hover:bg-white  rounded-md ">Host Room</button>
           <div className="flex flex-col justify-center gap-2 w-[280px] p-8 border-white border-2 rounded-xl">
           <div>
                <legend className="text-white">Nickname</legend>
                <input onChange={(e)=>{setNickname(e.target.value)}} className="border-2 bg-transparent px-3 py-1 rounded-2xl text-white" type="text" placeholder="Your name"/>
            </div>
            <div>
                <legend className="text-white">Room Id</legend>
                <input onChange={(e) =>{setRoomId(e.target.value)}} className="border-2 bg-transparent px-3 py-1 rounded-2xl text-white" type="text" placeholder="#01234"/>
            </div>
             <button onClick={JoinRoom} className="text-white border-2 mt-4 px-8 py-1  text-2xl transition-colors duration-300 hover:text-slate-600 hover:bg-white  rounded-md ">Join</button>
